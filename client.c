...
void handle_user_input(connection_info *connection)
{
  ...
  if(strcmp(input, "/q") == 0 || strcmp(input, "/quit") == 0){
    ...
    puts("/broadcast or /b: Send a public message to all users.");
  }
  ...
  //regular public message
    ...
    puts(KRED "Type /h to see the right format" RESET);
    return;
  }
...
int main(int argc, char *argv[])
{
  ...
  while(true)
  {
    ...
    //Add the STDIN_FILENO and connection socket to file_descriptors
    ...
    //Handle input from users(stdin)
    if(FD_ISSET(STDIN_FILENO, &file_descriptors)){
      handle_user_input(&connection);
    }

    //Hanle message from servers
